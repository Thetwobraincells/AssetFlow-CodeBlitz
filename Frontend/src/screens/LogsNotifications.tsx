...
const notifMeta: Record<string, { icon: LucideIcon; accent: string; label: string }> = {
...
const DEFAULT_NOTIF = { icon: Bell, accent: "slate", label: "Notification" };
...
function NotificationsPanel() {
  ...
  const notifFilters = ["All", "Unread", ...Object.keys(notifMeta)];
    ...
    <div className="space-y-4">
        ...
        >
          {notifFilters.map((f) => (
            <option key={f} value={f}>
              {f === "All" ? "All Types" : f === "Unread" ? `Unread Only (${unreadCount})` : (notifMeta[f]?.label ?? f)}
            </option>
          ))}
        </select>
      ...
      <div className="bg-surface border border-border rounded-md overflow-hidden">
        <div className="divide-y divide-border">
          {filtered.map((n) => {
            const meta = notifMeta[n.type] || DEFAULT_NOTIF;
...
// Static fallback — no dedicated backend endpoint for activity log
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="bg-surface border border-border rounded-md px-3 py-2 text-sm text-text focus:outline-none"
        >
          {entityTypes.map((t) => <option key={t} value={t}>{t === "All" ? "Entity: All" : t}</option>)}
        </select>
      </div>
